...
socket.on("chunk", (data) => {
    ...
    Object.keys(hash).forEach((k) => {
        ...
        if (o.indexTotal == o.chunks.length) {
            o.chunks = o.chunks.map((x) => {x.index = parseInt(x.index); return x;});
            o.chunks.sort((a, b) => {return a.index < b.index ? -1 : 1});
...
(() => {
    ...
    inpFile.onchange = (event) => {
        ...
        getBase64(file).then((r) => {
            ...
            const metadata = {now: new Date().getTime() / 1000, name: file.name, size: r.data.length, type: file.type};
